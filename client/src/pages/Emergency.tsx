    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        <div className="w-20 h-20 bg-teal-100 rounded-full flex items-center justify-center mx-auto mb-6">
          <AlertTriangle size={36} className="text-teal-600" />
        </div>
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Emergency Alert Sent</h1>
        <p className="text-slate-600 mb-6">{result.message}</p>
        <div className="bg-white rounded-2xl border border-slate-200 p-5 text-left space-y-3 shadow-sm">
          <div className="flex justify-between"><span className="text-slate-500 text-sm">Emergency ID</span><span className="font-mono text-sm font-medium text-slate-900">{result.emergencyId}</span></div>
          <div className="flex justify-between"><span className="text-slate-500 text-sm">Est. Response</span><span className="font-medium text-teal-600">{result.estimatedResponse}</span></div>
          {result.nearestHospital && <div className="flex justify-between"><span className="text-slate-500 text-sm">Nearest Hospital</span><span className="font-medium text-slate-900">{result.nearestHospital}</span></div>}
        </div>
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4 mb-6">
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Your Location <span className="text-red-500">*</span></label>
          <input type="text" required value={form.location} onChange={e => setForm({ ...form, location: e.target.value })}
            ...
            className="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 transition" />
        </div>
        <div>
      ...
      <button onClick={() => trigger.mutate()} disabled={trigger.isPending || !form.location || !form.description}
        className="w-full bg-red-600 text-white py-4 rounded-2xl font-bold text-lg hover:bg-red-700 disabled:opacity-60 shadow-lg transition-colors">
